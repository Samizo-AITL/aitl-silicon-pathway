...
- Fully hardware-implement the **FSM layer** in the PID × FSM × LLM architecture
...
```
docs/chapter2/
├── README.md          ← Entry point of Chapter2
├── overview.md        ← Chapter overview and design flow
├── fsm_spec.md        ← Formal FSM specification
├── mapping_rules.md   ← Python-to-Verilog mapping rules
├── verilog_rtl.md     ← FSM RTL (skeleton → complete)
├── rtl_guidelines.md  ← ASIC / OpenLane RTL guidelines
└── assets/
    ├── fsm_diagram.mmd
    ├── state_table.csv
...
- **Chapter1**  
  ...
  └ Conceptual behavior and control logic  

- **Chapter2 (this chapter)**  
  Formal specification and RTL realization of the FSM  
  └ Hardware-level definition of states and transitions  

- **Chapter3 (planned)**  
  RTL simulation, synthesis, and ASIC flow  
  └ Verification using OpenLane  
